...
class cube(object):
    ...
    letCol = {'w':W, 'r':R, 'b':B, 'o':O, 'g':G, 'y':Y}
    FRONT = "F"
    BACK = "B"
    LEFT = "L"
    RIGHT = "R"
    ...
    def parseCube(self, f):
        ...
        for line in text_file:
            ...
            for _ in range(3):
                row = []
                for _ in range(3):
                    row.append(self.letCol[arr.pop(0)])
                face.append(row)
            faces.append(face)
            
        self.cube[self.UP].struc = faces[0]
        self.cube[self.LEFT].struc = faces[1]
        self.cube[self.FRONT].struc = faces[2]
        self.cube[self.RIGHT].struc = faces[3]
        self.cube[self.BACK].struc = faces[4]
        self.cube[self.DOWN].struc = faces[5]
...
if __name__ == "__main__":
    ...
    print "Type o to print orientation of the cube"
    print "Type v to view the cube"
    print "Type t to turn a face"
    print "Type m to turn the middle of a face"
    print "Type r to randomize the cube"
    print "Type s to solve the cube"
    print "Type i to import cube from file"
...
"""
    ...
    while command != "q":
        ...
        if command == "v":
            ...
            c.randomize()
            print "Cube randomizecd"
        elif command == "s":
            c.solveCube()
        elif command == "t":
            print "Which face do you want to turn? (F, B, L, R, U, D)?"
            turn_face = raw_input(">>>> ")[0].upper()
            print "Clockwise or counterclockwise? (Y for clockwise, N for counterclockwise)"
            clockwise = raw_input(">>>> ")[0].lower()
            if (turn_face in ['F', 'B', 'L', 'R', 'U', 'D'] and clockwise in ['y', 'n']):
                if clockwise == 'y':
                    clockwise = True
                else:
                    clockwise = False
                c.turn(turn_face, clockwise)
                print c.strCube()
            else:
            ...
            print "Which face do you want to turn? (F, B, L, R, U, D)?"
            turn_face = raw_input(">>>> ")[0].upper()
            if (turn_face in ['F', 'B', 'L', 'R', 'U', 'D']):
                c.turnMiddle(turn_face)
                print c.strCube()
            else:
                print "Cannot read input. Try again"
        elif command == 'o':
            print orientation
        elif command == 'i':
